...
double *init_a(int M, int N, double h1, double h2, double eps, Args args) {
  ...
  for (int k = 0; k < (N+2) * (M+2); ++k) {
    ...
    j = (k / (M+2));
    Point p1;
    p1.x = i * h1 - 0.5 * h1 + args.A1_field, p1.y = j * h2 - 0.5 * h2 + args.A2_field;
    Point p2;
    p2.x = i * h1 - 0.5 * h1 + args.A1_field, p2.y = j * h2 + 0.5 * h2 + args.A2_field;
  ...
  //     Point p2;
  //     p2.x = i * h1 - 0.5 * h1 + args.A1_field, p2.y = j * h2 + 0.5 * h2 + args.A2_field;
  //     a[(N+1)*(M+2)+i] = calc_a_ij(h2, p1, p2, eps);
  //   }
  // }
...
int main(int argc, char *argv[]) {
...

  {
    ...
    alpha_k = scalar_product(z_k_0, r_k_0, h1, h2, args.M_field + 2, args.N_field + 2, args) /
              scalar_product(temp3, p_k_1, h1, h2, args.M_field + 2, args.N_field + 2, args);
  ...
  for (int r = 0; r < args.world_size; ++r) {
        if (r == args.rank2d) {
            printf("[rank2d=%2d coords=(%d,%d) dims=(%d,%d)] "
                   "L=%2d R=%2d U=%2d D=%2d\n",
                   args.rank2d, args.coords[0], args.coords[1], args.dims[0], args.dims[1],
