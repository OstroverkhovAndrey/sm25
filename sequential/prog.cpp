...
int main(int argc, char *argv[]) {
  ...
  double *w_k = mat_create(M + 2, N + 2);
  double *w_k_plus1 = mat_create(M + 2, N + 2);

  double *temp1 = mat_create(M + 2, N + 2);
  double *temp2 = mat_create(M + 2, N + 2);
  double *temp3 = mat_create(M + 2, N + 2);
  double *temp4 = mat_create(M + 2, N + 2);

  double *z_k_0 = mat_create(M + 2, N + 2);
  double *p_k_1 = mat_create(M + 2, N + 2);
  double *r_k_0 = mat_create(M + 2, N + 2);
  double *z_k_1 = mat_create(M + 2, N + 2);
  double *p_k_2 = mat_create(M + 2, N + 2);
  double *r_k_1 = mat_create(M + 2, N + 2);

  {
    ...
    B_fun(F, M, N, temp2);
    mat_minus(temp2, temp1, M + 2, N + 2, r_k_0);

    // z_k = r_k / D;
    D_fun(a, b, r_k_0, M, N, h1, h2, z_k_0);

    // p_k_1 = z_k_0;
    mat_copy(z_k_0, M + 2, N + 2, p_k_1);

    // alpha_k == (z_0, k_0) / (Ap_1, p_1)
    A_fun(a, b, p_k_1, M, N, h1, h2, temp3);
    alpha_k = scalar_product(z_k_0, r_k_0, h1, h2, M + 2, N + 2) /
              scalar_product(temp3, p_k_1, h1, h2, M + 2, N + 2);

    // w_1 == w_0 + alpha_k * p_1
    mat_copy(p_k_1, M + 2, N + 2, temp4);
    mat_mul_number(temp4, alpha_k, M + 2, N + 2);
    mat_plus(w_k, temp4, M + 2, N + 2, w_k_plus1);
  ...
  for (; stop_iter(i, err, args); ++i) {
    ...
    double betta = scalar_product(z_k_1, r_k_1, h1, h2, M + 2, N + 2) /
                   scalar_product(z_k_0, r_k_0, h1, h2, M + 2, N + 2);
    ...
    alpha_k = scalar_product(z_k_1, r_k_1, h1, h2, M + 2, N + 2) /
              scalar_product(temp2, p_k_2, h1, h2, M + 2, N + 2);
  ...
  mat_free(temp1, M + 2, N + 2);
  mat_free(temp2, M + 2, N + 2);
  mat_free(temp3, M + 2, N + 2);
  mat_free(temp4, M + 2, N + 2);

  mat_free(w_k, M + 2, N + 2);
  mat_free(w_k_plus1, M + 2, N + 2);
  mat_free(z_k_0, M + 2, N + 2);
  mat_free(p_k_1, M + 2, N + 2);
  mat_free(r_k_0, M + 2, N + 2);
  mat_free(z_k_1, M + 2, N + 2);
  mat_free(p_k_2, M + 2, N + 2);
  mat_free(r_k_1, M + 2, N + 2);
